...
- **`backend/` (V2 service)** — FastAPI with SQLAlchemy, optional GraphQL, Postgres + Redis via Docker, JWT for admin writes — see **[backend/README.md](backend/README.md)**
...
# Start the API (root SQLite-backed service)
pip install -r requirements.txt
# Build SQLite/CSV from JSON (creates data/sql/military_hardware.db — not in git)
...
# JSON
python3 -c "import json; data=json.load(open('data/json/platforms.json', encoding='utf-8')); print(f'{len(data)} platforms loaded')"
```

For the **Docker / Postgres `backend/`** service, see **[backend/README.md](backend/README.md)** and **[docker/README.md](docker/README.md)**.
...
│   ├── sql/military_hardware.db   # SQLite (generated by export; use .gitkeep in repo)
│   ├── migrations/                # Version migration scripts
│   └── sipri/                     # SIPRI external data (expenditure, companies, transfers)
├── backend/                       # V2 FastAPI: Alembic/Postgres, GraphQL, Docker profile
...
| **[backend/README.md](backend/README.md)** | V2 FastAPI service (REST, GraphQL, Postgres, local dev) |
| **[V2_0_IMPLEMENTATION.md](docs/V2_0_IMPLEMENTATION.md)** | V2.0 phased delivery closure & deferred backlog |
